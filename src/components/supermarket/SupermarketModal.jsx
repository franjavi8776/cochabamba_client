...
import { createComment, getCommentsBySupermarket } from "../../redux/actions";
import CarouselImages from "../carousel/Carrousel";
import Swal from "sweetalert2";
import PropTypes from "prop-types";

const SupermarketModal = ({ isOpen, onClose, supermarket }) => {
  ...
  useEffect(() => {
    dispatch(getCommentsBySupermarket(supermarket.id));
  ...
  const center = {
    lat: supermarket?.location.latitude || 0,
    lng: supermarket?.location.longitude || 0,
  };
    ...
    <div className="fixed inset-0 bg-primary dark:bg-neutral text-primary  dark:text-neutral bg-opacity-50 dark:bg-opacity-50 flex items-center justify-center z-50 ">
      <div className="bg-neutral dark:bg-primary w-[320px] md:w-[800px] lg:w-[1000px] max-h-[90vh] p-6 rounded-lg shadow-lg relative overflow-y-auto border border-primary dark:border-secondary">
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-10">
          <div>
            <CarouselImages images={supermarket.images} />
          ...
          <div>
            <div className="text-sm md:text-lg space-y-1 md:space-y-1.5">
              ...
              <p>
                ...
                <a
                  href={`https://wa.me/${supermarket.codArea}${supermarket.phone}`}
                  target="_blank"
                ...
                >
                  <i
                    ...
                    aria-hidden="true"
                  ></i>
                  <span>{`${supermarket.codArea} ${supermarket.phone}`}</span>
                </a>
              </p>
              <p>
                <small className="text-accent">Ofertas:</small>
                <span className="mx-2">{supermarket.offers[0]}</span>
              </p>
              <p>
                <small className="text-accent">Zona:</small> {supermarket.zone}
              </p>
              <p>
                <small className="text-accent">Ciudad:</small>
                <span className="mx-2">{supermarket.city}</span>
              </p>
              <p>
                <small className="text-accent">Pais:</small>
                <span className="mx-2">{supermarket.country}</span>
              </p>
              <p>
                <small className="text-accent">Lun-Vie:</small>
                <span className=" mx-2">{supermarket.time.weekdays}</span>
                <br />
                <small className="text-accent">Sab-Dom:</small>
                <span className=" mx-2">{supermarket.time.weekends}</span>
              </p>
              <p>
                <small className="text-accent">Web: </small>
                <a
                  href={supermarket.web}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-green-500"
                >
