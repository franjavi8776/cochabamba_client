...
import { createSupermarket, updateSupermarket } from "../../redux/actions";
...
const SupermarketForm = () => {
  ...
  const { supermarketState } = location.state || {};

  const [images, setImages] = useState([]);
  const [offerInput, setOfferInput] = useState("");
  const [categoryInput, setCategoryInput] = useState("");
  const [supermarket, setSupermarket] = useState({
    id: supermarketState?.id || null,
    name: supermarketState?.name || "",
    location: supermarketState?.location || {
      latitude: "",
      longitude: "",
    },
    images: supermarketState?.images
      ? supermarketState.images.map((image) =>
          typeof image === "string" ? { file: null, preview: image } : image
        )
      : [],
    offers: supermarketState?.offers || [],
    codArea: supermarketState?.codArea || "591",
    phone: supermarketState?.phone || "",
    city: supermarketState?.city || "",
    country: supermarketState?.country || "",
    web: supermarketState?.web || "",
    time: supermarketState?.time || {
      weekdays: "",
      weekends: "",
    },
    zone: supermarketState?.zone || "Central",
    categories: supermarketState?.categories || [],
    isActive: supermarketState?.isActive || true,
    user_id: supermarketState?.user_id || "",
  });
  ...
  const handleImageChange = (e) => {
    ...
    if (supermarket.images.length + files.length > 10) {
  ...
  const handleSubmit = (e) => {
    ...
    const existingImages = supermarket.images
      .filter((img) => !img.file)
      .map((img) => img.preview || img); // Filtra solo URLs

    const newImages = supermarket.images.filter((img) => img.file);

    if (supermarket.id) {
      dispatch(
        updateSupermarket(
          supermarket.id,
          { ...supermarket, images: existingImages },
          newImages,
          id
        )
      );
      ...
      dispatch(createSupermarket(supermarket, newImages, id));
      Swal.fire({
        position: "top",
        ...
        >
          ...
          <div className="w-[100%] h-[200px] mt-[10px]">
            {isLoaded ? (
              <GoogleMap
                location={{
                  latitude: String(supermarket.location.latitude),
                  longitude: String(supermarket.location.longitude),
                }}
          ...
          <div className="w-[100%] flex flex-col mb-3 text-xs md:text-lg">
            <div className="w-[100%] flex mb-3 ">
              ...
              <input
                ...
                id="time-weekdays"
                name="time-weekdays"
                value={supermarket.time.weekdays}
                onChange={(e) =>
                  setSupermarket({
                    ...supermarket,
                    time: { ...supermarket.time, weekdays: e.target.value },
                  })
                }
            ...
            <div className="w-[100%] flex">
              ...
              <input
                ...
                id="time-weekends"
                name="time-weekends"
                value={supermarket.time.weekends}
                onChange={(e) =>
                  setSupermarket({
                    ...supermarket,
                    time: { ...supermarket.time, weekends: e.target.value },
                  })
                }
                className="w-[75%] rounded-md p-1  border border-black"
                type="text"
          ...
          <div className="w-[100%] text-xs md:text-lg">
            <ul className="bg-secondary">
              {supermarket.offers.map((offer, index) => (
          ...
          <div className="h-22 flex items-center ">
            ...
            <div className="w-[60%] md:w-[80%] flex flex-wrap gap-4 pl-10">
              {supermarket.images.map((image, index) => (
          ...
          <div className="text-center mt-8 text-xs md:text-lg">
            <button
              ...
              type="submit"
            >
              {supermarketState?.id ? "Actualizar" : "Registrar"}
            </button>
          </div>
        </form>
      </div>
