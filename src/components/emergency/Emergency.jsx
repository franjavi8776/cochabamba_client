...
const Emergency = () => {
  ...
  const emergenciesByCategory = useSelector(
    (state) => state.emergenciesByCategory
  );

  const error = useSelector((state) => state.error);

  useEffect(() => {
    dispatch(getAllEmergencies(searchTerm, page, limit)).then(() =>
      setLoading(false)
    );
  ...
  const handleClickCategory = (category) => {
    setPage(1);
    if (category === "Todos") {
      dispatch(getAllEmergencies(searchTerm, page, limit));
    } else {
      dispatch(getEmergenciesByCategory([category], page, limit));
    }
  };
  ...
  return (
    <div className="w-full min-h-[100vh] text-primary bg-neutral dark:bg-primary dark:text-neutral">
      <Navbar nameCategory="Emergencias" />
      <div className="w-[85vw] md:w-[70vw] m-auto mt-10 flex  flex-wrap justify-center gap-5 pb-10 ">
        ...
        {error ? (
          ...
          <>
            {(emergenciesByCategory.length > 0
              ...
              (emergenciesByCategory.length > 0
                ...
                >
                  <img
                    key={emergency.id}
                    src={emergency.images[0]}
                    alt={emergency.name}
                    className="w-full h-[150px] object-cover object-center"
                  />
                  <div className="flex justify-between items-center">
                    <h1 className="text-sm md:text-xl m-3">{emergency.name}</h1>
                    <p className="m-3">
                      {emergency.averageStars ? (
                        renderStars(emergency.averageStars)
                      ) : (
