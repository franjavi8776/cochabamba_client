...
import { createComment, getCommentsByMovieTheater } from "../../redux/actions";
import CarouselImages from "../carousel/Carrousel";
import Swal from "sweetalert2";
import PropTypes from "prop-types";

const MovieTheaterModal = ({ isOpen, onClose, movieTheater }) => {
  ...
  useEffect(() => {
    dispatch(getCommentsByMovieTheater(movieTheater.id));
  ...
  const center = {
    lat: movieTheater?.location.latitude || 0,
    lng: movieTheater?.location.longitude || 0,
  };
    ...
    if (comment && stars) {
      dispatch(
        createComment({
          comments: comment,
          stars,
          movieTheater_id: movieTheater.id,
          user_id: id,
        })
      ).then(() => {
        dispatch(getCommentsByMovieTheater(movieTheater.id));
    ...
    <div className="fixed inset-0 bg-primary dark:bg-neutral text-primary  dark:text-neutral bg-opacity-50 dark:bg-opacity-50 flex items-center justify-center z-50 ">
      <div className="bg-neutral dark:bg-primary w-[320px] md:w-[800px] lg:w-[1000px] max-h-[90vh] p-6 rounded-lg shadow-lg relative overflow-y-auto border border-primary dark:border-secondary">
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-10">
          <div>
            <CarouselImages images={movieTheater.images} />
          ...
          <div>
            <div className="text-sm md:text-lg space-y-1 md:space-y-1.5">
              <h1>
                ...
                <span>{movieTheater.name}</span>
              </h1>
              <p>
                <small className="text-accent">Whatsapp:</small>
                <a
                  href={`https://wa.me/${movieTheater.codArea}${movieTheater.phone}`}
                  target="_blank"
                ...
                >
                  <i
                    ...
                    aria-hidden="true"
                  ></i>
                  <span>{`${movieTheater.codArea} ${movieTheater.phone}`}</span>
                </a>
              </p>
              <p>
                <small className="text-accent">Ofertas:</small>
                <span className="mx-2">{movieTheater.offers[0]}</span>
              </p>
              <p>
                <small className="text-accent">Zona:</small> {movieTheater.zone}
              </p>
              <p>
                <small className="text-accent">Ciudad:</small>
                <span className="mx-2">{movieTheater.city}</span>
              </p>
              <p>
                <small className="text-accent">Pais:</small>
                <span className="mx-2">{movieTheater.country}</span>
              </p>
              <p>
                <small className="text-accent">Lun-Vie:</small>
                <span className=" mx-2">{movieTheater.time.weekdays}</span>
                <br />
                <small className="text-accent">Sab-Dom:</small>
                <span className=" mx-2">{movieTheater.time.weekends}</span>
              </p>
              <p>
                <small className="text-accent">Web: </small>
                <a
                  href={movieTheater.web}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-secondary"
                >
...
MovieTheaterModal.propTypes = {
  isOpen: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  movieTheater: PropTypes.shape({
...
export default MovieTheaterModal;
