...
import { createHotel, updateHotel } from "../../redux/actions";
...
const HotelForm = () => {
  ...
  const [hotel, setHotel] = useState({
    id: hotelState?.id || null,
    name: hotelState?.name || "",
    location: hotelState?.location || {
      latitude: "",
      longitude: "",
    },
    images: hotelState?.images
      ? hotelState.images.map((image) =>
          typeof image === "string" ? { file: null, preview: image } : image
        )
      : [],
    offers: hotelState?.offers || [],
    codArea: hotelState?.codArea || "591",
    phone: hotelState?.phone || "",
    city: hotelState?.city || "",
    country: hotelState?.country || "",
    web: hotelState?.web || "",
    time: hotelState?.time || {
      weekdays: "",
      weekends: "",
    },
    zone: hotelState?.zone || "Central",
    categories: hotelState?.categories || [],
    isActive: hotelState?.isActive || true,
    user_id: hotelState?.user_id || "",
  });
  ...
  const handleImageChange = (e) => {
    ...
    if (hotel.images.length + files.length > 10) {
  ...
  const handleSubmit = (e) => {
    ...
    const newImages = hotel.images.filter((img) => img.file);
    ...
    if (hotel.id) {
      ...
      dispatch(createHotel(hotel, newImages, id));
      Swal.fire({
        position: "top",
        ...
        >
          ...
          <div className="w-[100%] h-[200px] mt-[10px]">
            {isLoaded ? (
              <GoogleMap
                location={{
                  latitude: String(hotel.location.latitude),
                  longitude: String(hotel.location.longitude),
                }}
          ...
          <div className="w-[100%] flex flex-col mb-3 text-xs md:text-lg">
            <div className="w-[100%] flex mb-3 ">
              ...
              <input
                ...
                id="time-weekdays"
                name="time-weekdays"
                value={hotel.time.weekdays}
                onChange={(e) =>
                  setHotel({
                    ...hotel,
                    time: { ...hotel.time, weekdays: e.target.value },
                  })
                }
            ...
            <div className="w-[100%] flex">
              ...
              <input
                ...
                id="time-weekends"
                name="time-weekends"
                value={hotel.time.weekends}
                onChange={(e) =>
                  setHotel({
                    ...hotel,
                    time: { ...hotel.time, weekends: e.target.value },
                  })
                }
                className="w-[75%] rounded-md p-1  border border-black"
                type="text"
