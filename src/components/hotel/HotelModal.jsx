...
import { createComment, getCommentsByHotel } from "../../redux/actions";
...
const HotelModal = ({ isOpen, onClose, hotel }) => {
  ...
  useEffect(() => {
    dispatch(getCommentsByHotel(hotel.id));
  }, [dispatch, hotel.id]);
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-10">
          ...
          <div>
            <div className="text-sm md:text-lg space-y-1 md:space-y-1.5">
              <h1>
                <small className="text-accent">Hotel:</small>{" "}
                <span>{hotel.name}</span>
              </h1>
              <p>
                <small className="text-accent">Reservas:</small>
                <a
                  href={`https://wa.me/${hotel.codArea}${hotel.phone}`}
                  target="_blank"
                ...
                >
                  <i
                    ...
                    aria-hidden="true"
                  ></i>
                  <span>{`${hotel.codArea} ${hotel.phone}`}</span>
                </a>
              </p>
              <p>
                <small className="text-accent">Ofertas:</small>
                <span className="mx-2">{hotel.offers[0]}</span>
              </p>
              <p>
                <small className="text-accent">Zona:</small> {hotel.zone}
              </p>
              <p>
                <small className="text-accent">Ciudad:</small>
                <span className="mx-2">{hotel.city}</span>
              </p>
              <p>
                <small className="text-accent">Pais:</small>
                <span className="mx-2">{hotel.country}</span>
              </p>
              <p>
                <small className="text-accent">Lun-Vie:</small>
                <span className=" mx-2">{hotel.time.weekdays}</span>
                <br />
                <small className="text-accent">Sab-Dom:</small>
                <span className=" mx-2">{hotel.time.weekends}</span>
              </p>
              <p>
            ...
            <div>
              <form onSubmit={handleCommentSubmit} className="mt-4 text-primary">
                <textarea
