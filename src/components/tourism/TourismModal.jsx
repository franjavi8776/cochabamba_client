...
import { createComment, getCommentsByTourism } from "../../redux/actions";
import CarouselImages from "../carousel/Carrousel";
import Swal from "sweetalert2";
import PropTypes from "prop-types";

const TourismModal = ({ isOpen, onClose, tourism }) => {
  ...
  useEffect(() => {
    dispatch(getCommentsByTourism(tourism.id));
  }, [dispatch, tourism.id]);
  ...
  const center = {
    lat: tourism?.location.latitude || 0,
    lng: tourism?.location.longitude || 0,
  };
    ...
    <div className="fixed inset-0 bg-primary dark:bg-neutral text-primary  dark:text-neutral bg-opacity-50 dark:bg-opacity-50 flex items-center justify-center z-50 ">
      <div className="bg-neutral dark:bg-primary w-[320px] md:w-[800px] lg:w-[1000px] max-h-[90vh] p-6 rounded-lg shadow-lg relative overflow-y-auto border border-primary dark:border-secondary">
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-10">
          <div>
            <CarouselImages images={tourism.images} />
          ...
          <div>
            <div className="text-sm md:text-lg space-y-1 md:space-y-1.5">
              ...
              <p>
                ...
                <a
                  href={`https://wa.me/${tourism.codArea}${tourism.phone}`}
                  target="_blank"
                ...
                >
                  <i
                    ...
                    aria-hidden="true"
                  ></i>
                  <span>{`${tourism.codArea} ${tourism.phone}`}</span>
                </a>
              </p>
              <p>
                <small className="text-accent">Ofertas:</small>
                <span className="mx-2">{tourism.offers[0]}</span>
              </p>
              <p>
                <small className="text-accent">Zona:</small> {tourism.zone}
              </p>
              <p>
                <small className="text-accent">Ciudad:</small>
                <span className="mx-2">{tourism.city}</span>
              </p>
              <p>
                <small className="text-accent">Pais:</small>
                <span className="mx-2">{tourism.country}</span>
              </p>
              <p>
                <small className="text-accent">Lun-Vie:</small>
                <span className=" mx-2">{tourism.time.weekdays}</span>
                <br />
                <small className="text-accent">Sab-Dom:</small>
                <span className=" mx-2">{tourism.time.weekends}</span>
              </p>
              <p>
